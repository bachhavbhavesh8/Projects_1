...
class Stack:
    ...
    #  Push method (adding element to stack)
    def push(self, item):
        self.items.append(item)
        print(item, "pushed into stack")

    #  Pop method (removing element from stack)
    def pop(self):
        if len(self.items) == 0:
...
class Queue:
    ...
    def dequeue(self):
        if len(self.items) == 0:          # Check if queue is empty
...
# Change department from all employees
...
# Calculate salary with overtime for all employee
