...
Original file is located at
    https://colab.research.google.com/drive/1y_J6qmmkhVCWhQcaZ4S7JhnaeTgH-Ms_
"""
...
def check_range(num, start, end):
    if num >= start and num <= end:                   # if start <= num <= end:
        return True
    else:
        return False
...
unique_element(set([1,2,3,3,3,3,4,5]))
...
print(add(10))      # 10 + 15 = 25
print(multiply(6, 8))  # 6 * 8 = 48
...
# Sort using lambda (based on 'color')
sorted_list = sorted(lst, key=lambda x: x['color'])              # lambda x: x['color'] → tells Python sorting rule     # Works like: “sort by this key”
...
check_string = lambda s: (
    any(ch.isupper() for ch in s) and   # at least one uppercase
    any(ch.islower() for ch in s) and   # at least one lowercase
    any(ch.isdigit() for ch in s) and   # at least one number
    len(s) >= 8                        # minimum length
)
